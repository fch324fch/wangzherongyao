...
const Mingwen = require('../../models/Mingwen')

module.exports = app => {
    const router = require('express').Router()
    const mongoose = require('mongoose')
    //因为之前在mongoose用require-all引入了所有模型所以可以这么写
    const Category = mongoose.model("Category")
    const Article = mongoose.model("Article")
    const Hero = mongoose.model("Hero")
    const UserSkill = mongoose.model('UserSkill')
    const Item = mongoose.model('Item')
    const Mingwen = mongoose.model('Mingwen')
    //导入新闻
    router.get('/news/init', async (req, res) => {
        ...
        //获取Category中纯粹(lean()方法)的数据
        const cats = await Category.find({
            parent: parent
        }).lean()
        const newsTitles = ["周年庆返场皮肤票选规则优化，武陵仙君最新消息【老亚瑟的答疑时间】", "【周年庆皮肤】裴擒虎 x 李小龙！梦幻联动，同心筑梦", "王者风物志 | 投稿盘点，绝美插画、创意周边一次看够！", "梦奇联名新品来啦！", "王者峡谷超有爱的中秋过节攻略", "10月14日净化游戏环境声明及处罚公告", "10月14日“演员”惩罚名单", "10月14日外挂专项打击公告", "10月13日体验服不停机更新公告", "10月13日体验服停机更新公告", "【金秋回馈 登录送好礼】活动公告", "“一”启幸运活动开启，概率得永久史诗皮肤", "金秋十月，福利不停，参与活动免费得永久皮肤", "【微信用户专属】微信小程序“游戏礼品站”关羽-武圣五虎将皮肤上线抽免单活动", "关羽-武圣携礼参见，活动皮肤免费得，全新中秋回城特效等你拿！", "2020年秋冬赛事战令全新升级，终极奖励韩信星元皮肤等你拿！", "2020年KPL秋季赛9月16日热血开赛，主场地域化全面升级", "2020年KPL秋季赛热血来袭，线下售票9月7日12:00开启！", "2020年KGL秋季赛选手大名单公布，9月13日该我上场！", "2020年KPL秋季赛大名单公布"]
        const newsList = newsTitles.map(title => {
            //获取一个随机的排序 slice(0)表示复制一遍
    ...
    router.get('/news/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            ...
            {
                $lookup: {
                    //关联哪个集合 因为之前Aricle模型没有设置默认是小写复数形式
                    from: "articles",
                    //Category的_id
                    localField: '_id',
                    //Article的categories
        ...
        //热门 通过文章的categories中的id关联查询Categories
        cats.unshift({
            name: "热门",
    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        const subCats = cats.map(v => v._id)
        //热门 通过英雄的categories中的id关联查询Categories
        cats.unshift({
            name: "热门",
    ...
    //召唤师技能详情
    router.get('/user_skill/:id', async (req, res) => {
        const data = await UserSkill.findById(req.params.id).populate('userskills').lean()
        res.send(data)
    })
    //物品详情
    router.get('/items/:id', async (req, res) => {
        const data = await Item.findById(req.params.id).lean()
        res.send(data)
    })
    //铭文详情
    router.get('/mingwen/:id', async (req, res) => {
        const data = await Mingwen.findById(req.params.id).lean()
        res.send(data)
    })
