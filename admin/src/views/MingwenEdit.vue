...
export default {
  ...
  methods: {
    ...
    async save() {
      let res;
      if (this.id) {
        res = await this.$http.put(`rest/mingwen/${this.id}`, this.model);
      } else {
        res = await this.$http.post("rest/mingwen", this.model);
      }
      //使用 router.push 方法。这个方法会向 history 栈添加一个新的记录，所以，当用户点击浏览器后退按钮时，则回到之前的 URL
      // this.$router.push("/mingwen/list");
      this.$message({
        type: "success",
        message: "保存成功",
      });
    ...
    async fetch() {
      //因为要接收id所用字符串模板
      const res = await this.$http.get(`rest/mingwen/${this.id}`);
